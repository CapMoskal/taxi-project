...
2. **Mobile-first, но десктоп теперь тоже в работе.** Мобильный вьюпорт —
   первичный, проверять в первую очередь. Но с 2026-07-27 в проекте
   появился десктоп-адаптив по референсу Яндекс.Такси (эпик 2a/2b/2c в
   `docs/roadmap.md`) — брейкпоинт `lg` (1024px). Задачи с десктоп-вёрсткой
   проверять глазами в ОБОИХ вьюпортах (мобильный + десктоп), не только в
   мобильном. Десктоп больше не «бонус на потом» — он часть требований.
